  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <span className="text-xs text-white/30 font-light tracking-widest shrink-0">0{index + 1}</span>
        <h3 className="text-base sm:text-lg font-cinematic text-white tracking-widest uppercase truncate">
          {project.title}
        </h3>
        <div className={`h-[1px] flex-1 transition-colors duration-300 ${hovered ? 'bg-white/30' : 'bg-white/10'}`} />
      </div>
      ...
      >
        <img
          ...
          className={`absolute inset-0 w-full h-full transition-all duration-700
            ${expanded
              ? 'object-contain bg-black scale-100 grayscale-0'
              : hovered
                ? 'object-cover scale-105 grayscale-0'
                : 'object-cover scale-100 grayscale'
            }`}
        />
...
const ProjectMediaCards = ({ project, startIndex }) => {
  ...
  return (
    <>
      {media.map((item, i) => (
        <MediaItemCard key={item.id} item={item} project={project} index={startIndex + i} />
      ))}
    </>
  );
};
...
export const MediaGallery = () => {
  ...
  useGSAP(() => {
    ScrollTrigger.create({
      ...
      start: 'top 75%',
      onEnter: () => {
        gsap.to('.media-headline', { opacity: 1, y: 0, duration: 1.2, ease: 'power4.out' });
        gsap.to('.media-row', {
          opacity: 1, x: 0, y: 0, duration: 1, stagger: 0.1, ease: 'power3.out', delay: 0.2,
        });
      },
    });
  ...
  useEffect(() => {
    ...
    const measureAndStart = () => {
      loopWidthRef.current = firstHalf.scrollWidth;
    ...
    const timer = setTimeout(measureAndStart, 300);
    return () => {
      clearTimeout(timer);
        ...
        >
          <div ref={firstHalfRef} className="flex gap-4 sm:gap-6 shrink-0">
            {allProjects.map((project, idx) => {
              ...
              cardIndex++;
              return (
                <ProjectMediaCards key={`first-${project.id}-${idx}`} project={project} startIndex={ci} />
              );
            })}
          </div>
          ...
          <div className="flex gap-4 sm:gap-6 shrink-0">
            {allProjects.map((project, idx) => (
              <ProjectMediaCards key={`second-${project.id}-${idx}`} project={project} startIndex={0} />
            ))}
          </div>
        </div>
      </div>
