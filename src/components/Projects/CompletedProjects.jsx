...
const ExpandedProjectContent = ({ project, activeMediaId, setActiveMediaId, isPlaying, setIsPlaying }) => {
      ...
      {youtubeLinks.length > 0 && (
        <>
            ...
            {youtubeLinks.map((video) => {
              ...
              return (
                <div key={video.id} className="flex flex-col gap-2">
                  <button
                    onClick={(e) => { e.stopPropagation(); setActiveMediaId(isActive ? null : video.id); setIsPlaying(false); }}
                  ...
                  >
                    <img src={thumbUrl} alt={video.type} className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ${isActive ? 'grayscale-0 scale-105' : 'grayscale group-hover/v:grayscale-0 group-hover/v:scale-105'}`} />
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-2.5 bg-gradient-to-t from-black/90 via-black/50 to-transparent">
                      <p className="text-[10px] text-white/80 leading-tight truncate uppercase">{video.type || 'Video'}</p>
                    </div>
                  </button>
                </div>
              )
          ...
          {activeVideoData && (() => {
            ...
            const thumbUrl = ytId ? `https://img.youtube.com/vi/${ytId}/maxresdefault.jpg` : '';
            return (
              <div className="relative w-full aspect-video bg-black/60 flex items-center justify-center border border-white/10 overflow-hidden mt-6">
                ...
                {/* Close */}
                <button
                  onClick={(e) => { e.stopPropagation(); setActiveMediaId(null); setIsPlaying(false) }}
      ...
      <div className="relative z-10 w-full max-w-[95vw] xl:max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-16">
        ...
        <div className="flex flex-col divide-y divide-white/[0.07]">
          {completedProjects.slice(0, 4).map((project, index) => {
            ...
            const pCoverImage = project.poster ? `${BASE_URL}${project.poster}` : ''
                ...
                >
                  ...
                  <div className="relative w-full lg:w-64 xl:w-80 aspect-[16/9] shrink-0 overflow-hidden bg-black/20">
                    ...
                    <span className="absolute bottom-3 left-4 text-[10px] text-white/70 tracking-widest font-semibold drop-shadow-md z-10 uppercase">
                      {pSeasonsCount > 0 ? `${pSeasonsCount} Seasons` : (pEpisodesCount > 0 ? `${pEpisodesCount} Episodes` : '')}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-3 flex-wrap">
                      {pReleaseYear && (
                        <span className="text-[10px] text-white/80 tracking-[0.25em] uppercase">{pReleaseYear}</span>
                      )}
                      {pGenre && (
                        <>
                          {pReleaseYear && <span className="w-px h-3 bg-white/40" />}
                          <span className="text-[10px] text-white/80 tracking-[0.2em] uppercase">{pGenre}</span>
                        </>
                      )}
                      {pRuntime && (
                        <>
                          <span className="w-px h-3 bg-white/40" />
                          <span className="text-[10px] text-white/80 tracking-[0.2em] uppercase">{pRuntime}</span>
                        </>
                      )}
                      {pSeasonsCount > 0 && (
                        <>
                          <span className="w-px h-3 bg-white/40" />
                          <span className="text-[10px] text-white/80 tracking-[0.2em] uppercase">{pSeasonsCount} {pSeasonsCount > 1 ? 'Seasons' : 'Season'}</span>
                        </>
                      )}
                      {pEpisodesCount > 0 && (
                        <>
                          <span className="w-px h-3 bg-white/40" />
                          <span className="text-[10px] text-white/80 tracking-[0.2em] uppercase">{pEpisodesCount} {pEpisodesCount > 1 ? 'Episodes' : 'Episode'}</span>
                        </>
                      )}
                    </div>
