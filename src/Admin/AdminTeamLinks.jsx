...
const AdminTeamLinks = ({ memberId }) => {
    ...
    if (loading) return <div className="text-[10px] text-neutral-400 uppercase tracking-widest mt-2">Loading links...</div>;
    ...
    return (
        <div className="mt-4 pt-4 border-t border-neutral-100 dark:border-[#222]">
            ...
            <h4 className="text-[10px] uppercase tracking-[0.2em] font-bold text-black dark:text-white mb-3">Social Links</h4>

            {/* List */}
            {links.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                    {links.map(link => (
                        <div key={link.id} className="inline-flex items-center gap-2 px-3 py-1 bg-neutral-100 dark:bg-[#222] rounded-full text-[11px] text-black dark:text-white">
                            <span className="font-bold capitalize">{link.platform}</span>
                            <span className="text-neutral-400">|</span>
                            <a href={link.url} target="_blank" rel="noreferrer" className="text-neutral-500 hover:text-black dark:hover:text-white truncate max-w-[100px]">
